...
function handlertag() {
    try{
        ...
        lazyscript.src = "https://cdn.jsdelivr.net/gh/ashishm1/infyload@main/ajaxinate-blockshop.js";
    }
    catch(err){
        console.log(err);
    }
...
function addAjax(){
    ...
    var productgrid = $('div.product-collection-grid.product-grid').find('.row');
    if(productgrid.length > 0) {
        var pgrid = productgrid[0]
        pgrid.setAttribute("id", "LazyLoader-Loop");
        orientation = 'div.product-collection-grid.product-grid';
        }
