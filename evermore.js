...
function handlertag() {
    try{
        ...
        lazyscript.src = "https://cdn.jsdelivr.net/gh/ashishm1/infyload@main/ajaxinate-evermore.js";
    }
    catch(err){
        console.log(err);
    }
...
function addAjax(){
    ...
    var productgrid = $('div.products')
    if(productgrid.length > 0) {
        var pgrid = productgrid[0]
        pgrid.setAttribute("id", "LazyLoader-Loop");
        var orientation = 'div.products';
        }
    
    ll = $('div.product-pagination');
    if (ll.length > 0) {
        pagegrid = ll[0]
