...
function handlertag() {
    try{
        ...
        lazyscript.src = "https://cdn.jsdelivr.net/gh/ashishm1/infyload@main/ajaxinate-motion.js";
    }
    catch(err){
        console.log(err);
    }
...
function addAjax(){
    ...
    var productgrid = $('div.grid--uniform')
    if(productgrid.length > 0) {
        var pgrid = productgrid[0]
        pgrid.setAttribute("id", "LazyLoader-Loop");
        var orientation = 'div.grid--uniform';
        // document.styleSheets
        // img.grid-product__image.lazyautosizes.lazyloaded

        var infyStyle = document.createElement("style")
        infyStyle.type = "text/css"
        infyStyle.appendChild(document.createTextNode(`img.grid-product__image.lazyautosizes.lazyloaded{
            opacity: 1;
        }`));
        document.getElementsByTagName("head")[0].appendChild(infyStyle)
