...
function handlertag() {
    try{
        ...
        lazyscript.src = "https://cdn.jsdelivr.net/gh/ashishm1/infyload@main/ajaxinate-mrparker.js";
    }
    catch(err){
        console.log(err);
    }
...
function addAjax(){
    ...
    var productgrid = $('#product-loop')
    if(productgrid.length > 0) {
        var pgrid = productgrid[0]
        pgrid.setAttribute("class", "LazyLoader-Loop");
        var orientation = '#product-loop';
        }
    
    ll = $('#pagination');
    if (ll.length > 0) {
        pagegrid = ll[0]
        pagegrid.setAttribute("class", "LazyLoader-Pagination")
        } 
